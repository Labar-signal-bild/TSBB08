...
corners = cHarrisNew .* RegMax; %Ger en bild med 1 på alla corners den hittar
cornerpoints = find(corners == 1); %Ger en vector med pixlar på alla vita corners
...
for i = 1:length(cornerpoints) %Gör om pixlar till x,y koordinater
    cornerpointx(i) = floor(cornerpoints(i)/398)+1;
    cornerpointy(i) = cornerpoints(i) - floor(cornerpoints(i)/398)*398;
end

cornerpointx = ceil(cornerpointx/398*400);
cornerpointy = ceil(cornerpointy/398*400);

%cornervalues = im(cornerpoints);
%cornermatrix = [cornervalues cornerpoints];

%deletecornerpoints = cornerpoints;
%j = 1;

%for i = 1:length(cornervalues)
 %   if cornermatrix(i,1) < 200
  %      deletecornerpixels(1,j) = cornermatrix(i,2);
 %       j = j + 1;
   % end
%end
